...
def map_source_and_department(dept_or_src: str) -> tuple[str, str]:
    """Map external department or CRIS source system to (source_system, canonical_department)."""
    raw = (dept_or_src or "").strip().upper()
    if raw in ("TMS", "ENGG", "ENGINEERING"):
        return SourceSystem.TMS.value, Department.ENGG.value
    if raw in ("SMMS", "SNT", "S&T", "SIGNAL"):
        return SourceSystem.SMMS.value, Department.SNT.value
    if raw in ("TDMS", "TRD", "TRACTION", "OHE"):
        return SourceSystem.TDMS.value, Department.TRD.value
    if raw in ("COA", "OPERATING", "TRAFFIC"):
        raise ValueError(
            f"COA/Operating is a traffic operations source and cannot request maintenance blocks: {dept_or_src}"
        )
    raise ValueError(f"Unsupported maintenance department or source system: {dept_or_src}")
...
class DemoGatewayService:
        ...
        try:
            ...
            if request.type == "maintenance_block":
                ...
                payload = MaintenanceCreate(
                    ...
                    external_id=str(request.request_id),
                    external_payload={
                        "department": request.department,
                        "type": request.type,
                        "train_id": request.train_id,
                ...
                maintenance = self.maintenance_service.create(db, payload, auto_commit=False)
                try:
                    self.prediction_service.predict(db, maintenance, auto_commit=False)
                except (KeyError, OSError, TypeError, ValueError):
                    # ML scoring failure must not roll back maintenance ingestion
                    pass
    ...
    def _make_decision(request: DemoRequest, maintenance_request_id: str | None) -> DemoDecision:
        if request.type == "maintenance_block":
            notes = "Maintenance block request queued for RailNexus AI optimization and review"
            if maintenance_request_id is None:
                notes = "Recorded in RailNexus integration ledger; maintenance request omitted"
            return DemoDecision(
        ...
        if request.type == "section_entry":
            return DemoDecision(
                ...
                resulting_state="clear",
                decided_at=datetime.now(timezone.utc),
                notes="Section-entry telemetry recorded by RailNexus gateway; pending dispatch authorization",
            )
        return DemoDecision(
            request_id=request.request_id,
            status="queued",
            ...
            resulting_state="clear",
            decided_at=datetime.now(timezone.utc),
            notes="Running-status telemetry recorded by RailNexus gateway",
        )

    @staticmethod
    def _decision(record: DemoIntegrationRequest) -> DemoDecision:
        if record.status not in {"queued", "received", "approved", "rerouted", "rejected"} or record.resulting_state is None or record.decided_at is None:
        ...
        return DemoDecision(
            ...
            decided_at=record.decided_at.replace(tzinfo=timezone.utc),
            notes="Idempotent replay of stored RailNexus decision",
        )

    @staticmethod
    def _model_features(request: DemoRequest, canonical_dept: str = "ENGG", source_system: str = "TMS") -> dict[str, object]:
        return {
            "asset_age_days": 2500,
            ...
            "is_rain_day": True,
            "request_hour": request.raised_at.hour,
            "request_day_of_week": request.raised_at.weekday(),
            "request_month": request.raised_at.month,
            "request_is_weekend": request.raised_at.weekday() >= 5,
            "location_km_marker": TRAINED_SECTION_KM[request.section_id],
            "window_train_count": 8,
            "planned_start_hour": request.raised_at.hour,
