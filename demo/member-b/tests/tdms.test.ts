...
describe("TDMS department", () => {
  it("raises a section-entry request for a section the train does not hold", () => {
    ...
    const requests = engine.raise(
      snapshotWith([makeTrain("12005", { nextSectionId: "BAR-YJ", heldSectionIds: ["UMB-BAR"] })]),
      engineCtx(),
      store,
    );
  ...
  it("does not raise when the train already holds the next section", () => {
    const store = makeStore();
    const requests = engine.raise(
      snapshotWith([makeTrain("12005", { nextSectionId: "BAR-YJ", heldSectionIds: ["UMB-BAR", "BAR-YJ"] })]),
      engineCtx(),
      store,
    );
  ...
  it("raises for a different pair once the train advances", () => {
    ...
    const advanced = engine.raise(
      snapshotWith([
        makeTrain("12005", { km: 247, nextSectionId: "YJ-YWS", heldSectionIds: ["UMB-BAR", "BAR-YJ"] }),
      ]),
      ctx,
      store,
    );
    expect(advanced).toHaveLength(1);
    expect(advanced[0]!.sectionId).toBe("YJ-YWS");
  });
});