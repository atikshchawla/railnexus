...
export class MidLayerClient {
  ...
  async submit(request: Request): Promise<Decision> {
    const response = await this.fetchImpl(`${this.baseUrl}/requests`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(request) });
    if (!response.ok) {
      const err = await response.json().catch(() => null) as { message?: string; error?: string } | null;
      throw new Error(`mid-layer request ${response.status}: ${err?.message || err?.error || response.statusText}`);
    }
